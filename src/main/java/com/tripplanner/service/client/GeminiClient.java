...
public class GeminiClient implements AiClient {
    ...
    public void initialize() {
        ...
        logger.info("Available Gemini keys: {}",
                apiKeyRotationService != null ? apiKeyRotationService.getAvailableKeyCount("gemini") : 0);
            ...
            if (mockMode) {
                ...
                if (candidates != null && candidates.isArray() && candidates.size() > 0) {
                    ...
                    if (parts != null && parts.isArray() && parts.size() > 0) {
                        ...
                        logger.info("=== FULL GENERATED CONTENT ===");
                        logger.info("{}", generatedText);
                        logger.info("=== END GENERATED CONTENT ===");
                        logger.info("=========================================");
     ...
     * Permanent errors indicate configuration or request problems that won't be
     * fixed by retrying.
     */
     ...
     * Note: This is kept for backward compatibility but retry logic is now in
     * ResilientAiClient.
     */
    ...
    private String getDefaultMockResponse() {
        try {
            ...
            java.nio.file.Path jsonPath = java.nio.file.Paths
                    .get("logs/gemini-responses/barcelona_3day_family_normalized.json");
    ...
    private void trackTokenUsage(JsonNode responseJson, long latencyMs) {
        ...
        try {
            ...
            int completionTokens = usageMetadata.has("candidatesTokenCount")
                    ...
                    : 0;
            int thoughtsTokens = usageMetadata.has("thoughtsTokenCount")
                    ? usageMetadata.get("thoughtsTokenCount").asInt()
                    : 0;
            int cachedTokens = usageMetadata.has("cachedContentTokenCount")
                    ? usageMetadata.get("cachedContentTokenCount").asInt()
                    : 0;
            int totalTokens = usageMetadata.has("totalTokenCount")
                    ? usageMetadata.get("totalTokenCount").asInt()
                    : (promptTokens + completionTokens);

            // Calculate cost (Gemini 2.5 Flash pricing: $0.075 input, $0.30 output per 1M tokens)
            // Cached tokens are cheaper: $0.01875 per 1M tokens (25% of input cost)
            double inputCost = (promptTokens / 1_000_000.0) * 0.075;
            double outputCost = (completionTokens / 1_000_000.0) * 0.30;
            double cachedCost = (cachedTokens / 1_000_000.0) * 0.01875;
            double totalCost = inputCost + outputCost + cachedCost;
            ...
            properties.put("agent", com.tripplanner.util.UserContext.getAgentName()); // Track which agent made the call
