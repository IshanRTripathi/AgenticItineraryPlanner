...
public class OpenRouterClient implements AiClient {
	...
	public String generateContent(String userPrompt, String systemPrompt) {
		...
		logger.info("Request details - Model: {}, Temperature: {}, Max tokens: {}", modelName, temperature, maxTokens);
	...
	private String attemptContentGenerationWithKey(String apiKey, String userPrompt, String systemPrompt) {
		try {
			...
			logger.info("Request payload size: {} chars", body.length());
			...
			logger.info("Response body size: {} chars", response.body().length());
	...
	public String generateStructuredContent(String userPrompt, String jsonSchema, String systemPrompt) {
		...
		logger.info("User prompt length: {} chars", userPrompt != null ? userPrompt.length() : 0);
		logger.info("System prompt length: {} chars", systemPrompt != null ? systemPrompt.length() : 0);
		logger.info("JSON schema length: {} chars", jsonSchema != null ? jsonSchema.length() : 0);
		logger.info("=== FULL USER PROMPT ===");
		logger.info("{}", userPrompt);
		if (systemPrompt != null && !systemPrompt.trim().isEmpty()) {
			logger.info("=== FULL SYSTEM PROMPT ===");
